...
public class CommentUtils {
    ...
    public static void checkReceivedMail(List<MailMessage> mails, DocumentModel commentDocModel,
        ...
        DocumentModel sourceDocument = ((DocumentEventContext) event.getContext()).getSourceDocument();

        if (COMMENT_ADDED.equals(commentEventType) && !commentedDocModel.getId().equals(sourceDocument.getId())) {
            expectedMailsCount = 2;
        }
        ...
        if (expectedMailsCount > 0) {
            ...
            assertEquals(expectedMailsCount, mailsBySubject.size());

            String expectedMailContent = getExpectedMailContent(commentDocModel, commentedDocModel, event,
                    commentEventType);
            assertEquals(expectedMailContent, getMailContent(mailsBySubject.get(mailsBySubject.size() - 1)));
        }
    }
    ...
    public static String getExpectedMailContent(DocumentModel commentDocModel, DocumentModel commentedDocModel,
        ...
        try {
            ...
            var model = Map.of("COMMENT_AUTHOR", commentDocModel.getPropertyValue(COMMENT_AUTHOR), //
                    ...
                    "COMMENT_SUBSCRIPTION_NAME", COMMENT_ADDED.equals(commentEventType) ? "New" : "Modification");
